...
export default function MyOrders() {
  ...
  const { user } = useAuth();
  const backendURL = import.meta.env.VITE_BACKEND_URL; // Lấy URL từ biến môi trường
  useEffect(() => {
    // Bước 1: Lấy danh sách đơn hàng
    axios
      .post(
        `${backendURL}/orders/mine`,
        {},
        {
          ...
          rawOrders.map(async (order) => {
            try {
              const detailRes = await axios.get(
                `${backendURL}/orders/details/${order._id}`,
                { withCredentials: true }
              );
              return {
                ...order,
          ...
          <ul className="product-orderlist">
            {order.products.map((item) => (
              <li key={item._id} className="product-item">
                <img
                  src={`${backendURL}${item.product.images}`}
